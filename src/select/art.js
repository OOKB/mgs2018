...
export const artGraph = getGraphSlice([
  "CreativeWork",
  "ImageObject",
  "MediaObject"
]);
...
export function expandGetFullEntities(graphSlice) {
  return mapValues(
    flow(
      getGraphNode(graphSlice),
      flow(
        buildFullEntity(1, graphSlice),
        artFill
      )
    )
  );
...
export const slideStudentsArtOnly = students =>
  reduce(
    students,
    (result, { art, id, name }) =>
      result.concat(
        map(art, item => ({ ...item, studentId: id, studentName: name }))
      ),
    []
  );
...
export const studentSplashArt = createSelector(
  ...
  flow(
    selectForm,
    get("refresh-art.value")
  ),
  sampleSize(7)
);

export const studentSlideArt = createSelector(
  createSelector(getStudentsArt, slideStudentsArtOnly),
  sampleSize(12)
);
